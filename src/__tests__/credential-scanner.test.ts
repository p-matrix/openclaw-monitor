...
describe('scanCredentials — 내장 패턴 11종', () => {
  ...
  test('[11] Stripe Secret Key (sk_live_) 탐지', () => {
    ...
    // Note: string split to avoid static secret-scanning false positives in CI
    const text = 'STRIPE_KEY=' + 'sk_live_' + 'abcdefghijklmnopqrstuvwx';
    const results = scanCredentials(text);
    expect(results).toHaveLength(1);
  ...
  test('[11] Stripe sk_test_ (언더스코어) — TEST_EXCLUSIONS "sk-test-"(대시)와 달라 탐지됨', () => {
    ...
    // Note: string split to avoid static secret-scanning false positives in CI
    const text = 'STRIPE_KEY=' + 'sk_test_' + 'abcdefghijklmnopqrstuvwx';
    const results = scanCredentials(text);
